...
def _paa(ts, nr_windows):
	window_means = []
	window_size = int(np.ceil(float(len(ts)) / float(nr_windows)))
...
def _transform_ts(ts, nr_windows, alphabet_size, symbol_map):
...
def transform(timeseries, nr_windows, alphabet_size):
	...
	symbol_map[(split_points[-1], float('inf'))] = len(split_points) + 1

	transformed_ts = [_transform_ts(ts, nr_windows, alphabet_size, symbol_map) 
