...
for dataset in metadata:
    ...
    print('Learning shapelets + CLF on dataset {} || Accuracy = {}'.format(
        dataset['train']['name'], accuracy_score(y_test, clf.predict(X_test))
    ))
    ...
    print('Learning shapelets + RF on dataset {} || Accuracy = {}'.format(
        dataset['train']['name'], accuracy_score(y_test, rf.predict(X_distances_test))
    ))
    """

    genetic_extractor = MultiGeneticExtractor(verbose=True, population_size=20, iterations=500, wait=10, plot=True)
    shap_transformer = ShapeletTransformer(method=genetic_extractor, min_len=3, max_len=X_train.shape[1], nr_shapelets=X_train.shape[1]//2, metric='ig')
    start = time.time()
    ...
    result_vectors.append([dataset['train']['name']] + [genetic_time, genetic_accuracy, learning_time, learning_accuracy])

results_df = pd.DataFrame(result_vectors)
results_df.columns = ['Dataset Name', 'Genetic Time', 'Genetic Accuracy', 'Learning Time', 'Learning Accuracy']
